...
export default function Monitors() {
  ...
  const untagged = { id: 0, name: t("monitors.untagged"), color: "#64748b" };
  const groups = useMemo(() => {
    const filtered = q ? monitors.filter((m) => m.name.toLowerCase().includes(q.toLowerCase())) : monitors;
    const out = tags.map((tag) => ({ tag, items: filtered.filter((m) => m.tags?.some((x) => x.id === tag.id)) })).filter((g) => g.items.length);
    const rest = filtered.filter((m) => !m.tags?.length);
    if (rest.length) out.push({ tag: untagged, items: rest });
    return out;
  }, [monitors, tags, q, untagged.name]);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-end justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-fg">{t("monitors.title")}</h1>
          <p className="text-sm text-muted mt-1">{t("monitors.subtitle")}</p>
        </div>
      ...
      {loading ? (
        ...
        <p className="card p-8 text-center text-sm text-muted">{t("monitors.empty")}</p>
      ) : (
          ...
          return (
            <section key={tag.id} className="card overflow-hidden">
              <button onClick={() => toggle(tag.id)} className="w-full flex items-center gap-3 px-5 py-3.5 text-left hover:bg-panel2/60 transition-colors">
                ...
                <span className="text-xs text-muted">{t("monitors.count", { n: items.length })}</span>
