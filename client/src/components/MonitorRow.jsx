...
import { Wrench, Globe2, Network } from "lucide-react";
    ...
    >
      <div className="min-w-0">
        <div className="flex items-center gap-2.5 flex-wrap">
          ...
          {m.alert_suppressed && (
            <span
              className="inline-flex items-center gap-1 text-[10px] text-muted border border-border rounded px-1.5 py-0.5"
              title={t("dep.blockedBy", { parent: m.blocked_by?.name || "" })}
            >
              <Network size={11} /> {t("dep.badge")}
            </span>
          )}
